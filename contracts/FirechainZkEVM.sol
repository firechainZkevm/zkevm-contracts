...
import "./interfaces/IFirechainZkEVMGlobalExitRoot.sol";
import "@openzeppelin/contracts-upgradeable/access/OwnableUpgradeable.sol";
import "./interfaces/IFirechainZkEVMBridge.sol";
import "./lib/EmergencyManager.sol";
import "./interfaces/IFirechainZkEVMErrors.sol";
 ...
 */
contract FirechainZkEVM is
    OwnableUpgradeable,
    EmergencyManager,
    IFirechainZkEVMErrors
{
    ...
    IFirechainZkEVMGlobalExitRoot public immutable globalExitRootManager;
     ...
     */
    constructor(
        IFirechainZkEVMGlobalExitRoot _globalExitRootManager,
    ...
    /**
     * @notice Allows to halt the FirechainZkEVM if its possible to prove a different state root given the same batches
     * @param initPendingStateNum Init pending state, 0 if consolidated state is used
     * @param finalPendingStateNum Final pending state, that will be used to compare with the newStateRoot
     * @param initNumBatch Batch which the aggregator starts the verification
     * @param finalNewBatch Last batch aggregator intends to verify
    ...
    /**
     * @notice Function to activate emergency state, which also enables the emergency mode on both FirechainZkEVM and FirechainZkEVMBridge contracts
    ...
    /**
     * @notice Internal function to activate emergency state on both FirechainZkEVM and FirechainZkEVMBridge contracts
     */
