...
The first step is deploying and verifying the `FirechainZkEVMDeployer`, this will be the factory for deterministic contracts, the address of the contracts will depend on the `salt` and the `initialZkEVMDeployerOwner`
...
-   `consensusContract`: string, Consensus contract name of the new rollup deployed, current options are: "FirechainZkEVMEtrog","FirechainZkEVMV2","FirechainDataComittee", "FirechainDataComitteeEtrog",
...
-   `dataAvailabilityProtocol`: string, Data availability protocol, only mandatory/used when consensus contract is a Validiums, currently the only supported value is: `FirechainDataCommittee`
