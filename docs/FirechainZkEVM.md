...
```solidity
  function initialize(
    struct FirechainZkEVM.InitializePackedParameters initializePackedParameters,
    bytes32 genesisRoot,
...
| Name | Type | Description                                                          |
| :--- | :--- | :------------------------------------------------------------------- |
|`initializePackedParameters` | struct FirechainZkEVM.InitializePackedParameters | Struct to save gas and avoid stack too deep errors
...
```solidity
  function sequenceBatches(
    struct FirechainZkEVM.BatchData[] batches,
    address l2Coinbase
  ) external
```
...
| Name | Type | Description                                                          |
| :--- | :--- | :------------------------------------------------------------------- |
|`batches` | struct FirechainZkEVM.BatchData[] | Struct array which holds the necessary data to append new batches to the sequence
...
```solidity
  function sequenceForceBatches(
    struct FirechainZkEVM.ForcedBatchData[] batches
  ) external
```
...
| Name | Type | Description                                                          |
| :--- | :--- | :------------------------------------------------------------------- |
|`batches` | struct FirechainZkEVM.ForcedBatchData[] | Struct array which holds the necessary data to append force batches
...
```
Allows to halt the FirechainZkEVM if its possible to prove a different state root given the same batches
...
```
Function to activate emergency state, which also enables the emergency mode on both FirechainZkEVM and FirechainZkEVMBridge contracts
...
```
Internal function to activate emergency state on both FirechainZkEVM and FirechainZkEVMBridge contracts
