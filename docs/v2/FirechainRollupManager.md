...
```solidity
  function initialize(
    ...
    contract FirechainZkEVMExistentEtrog firechainZkEVM,
...
|`firechainZkEVM` | contract FirechainZkEVMExistentEtrog | New deployed Firechain zkEVM which will be initialized wiht previous values
...
```solidity
  function addExistingRollup(
    contract IFirechainRollupBase rollupAddress,
    contract IVerifierRollup verifier,
    uint64 forkID,
    uint64 chainID,
    bytes32 genesis,
...
| Name | Type | Description                                                          |
| :--- | :--- | :------------------------------------------------------------------- |
|`rollupAddress` | contract IFirechainRollupBase | Rollup address
...
```solidity
  ...
  ) internal returns (struct FirechainRollupManager.RollupData rollup)
```
Add an already deployed rollup
note that this rollup does not follow any rollupType
...
```solidity
  function _verifyAndRewardBatches(
    struct FirechainRollupManager.RollupData rollup,
    uint64 pendingStateNum,
    uint64 initNumBatch,
    uint64 finalNewBatch,
    bytes32 newLocalExitRoot,
...
| Name | Type | Description                                                          |
| :--- | :--- | :------------------------------------------------------------------- |
|`rollup` | struct FirechainRollupManager.RollupData | Rollup Data storage pointer that will be used to the verification
|`pendingStateNum` | uint64 | Init pending state, 0 if consolidated state is used
|`initNumBatch` | uint64 | Batch which the aggregator starts the verification
|`finalNewBatch` | uint64 | Last batch aggregator intends to verify
|`newLocalExitRoot` | bytes32 | New local exit root once the batch is processed
...
| Name | Type | Description                                                          |
| :--- | :--- | :------------------------------------------------------------------- |
|`rollup` | struct FirechainRollupManager.RollupData | Rollup data storage pointer
|`pendingStateNum` | uint64 | Pending state to consolidate
...
| Name | Type | Description                                                          |
| :--- | :--- | :------------------------------------------------------------------- |
|`rollup` | struct FirechainRollupManager.RollupData | Rollup Data struct that will be checked
|`initPendingStateNum` | uint64 | Init pending state, 0 if consolidated state is used
|`finalPendingStateNum` | uint64 | Final pending state, that will be used to compare with the newStateRoot
|`initNumBatch` | uint64 | Batch which the aggregator starts the verification
|`finalNewBatch` | uint64 | Last batch aggregator intends to verify
...
```solidity
  function _updateBatchFee(
    struct FirechainRollupManager.RollupData newLastVerifiedBatch
  ) internal
```
...
| Name | Type | Description                                                          |
| :--- | :--- | :------------------------------------------------------------------- |
|`newLastVerifiedBatch` | struct FirechainRollupManager.RollupData | New last verified batch
...
```
Function to activate emergency state, which also enables the emergency mode on both FirechainRollupManager and FirechainZkEVMBridge contracts
...
```
Function to deactivate emergency state on both FirechainRollupManager and FirechainZkEVMBridge contracts
...
```
Internal function to activate emergency state on both FirechainRollupManager and FirechainZkEVMBridge contracts
