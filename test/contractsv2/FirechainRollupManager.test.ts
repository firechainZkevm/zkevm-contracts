...
import {
    ...
    TokenWrapped,
    Address,
    FirechainValidiumStorageMigration,
    FirechainDataCommittee,
    FirechainValidiumEtrogPrevious,
...
describe("Firechain Rollup Manager", () => {
    let deployer: any;
    let timelock: any;
    ...
    beforeEach("Deploy contract", async () => {
        ...
        // deploy mock verifier
        const FirechainRollupManagerFactory = await ethers.getContractFactory("FirechainRollupManagerMock");

        rollupManagerContract = (await upgrades.deployProxy(FirechainRollupManagerFactory, [], {
            initializer: false,
            constructorArgs: [
            ...
            unsafeAllow: ["constructor", "state-variable-immutable"],
        })) as unknown as FirechainRollupManagerMock;
    ...
    it("should check the initalized parameters", async () => {
        expect(await rollupManagerContract.globalExitRootManager()).to.be.equal(firechainZkEVMGlobalExitRoot.target);
        expect(await rollupManagerContract.pol()).to.be.equal(polTokenContract.target);
        expect(await rollupManagerContract.bridgeAddress()).to.be.equal(firechainZkEVMBridgeContract.target);
    ...
    it("should check full flow etrog", async () => {
        ...
        const FirechainZKEVMV2Factory = await ethers.getContractFactory("FirechainZkEVMEtrog");
        const FirechainZKEVMV2Contract = await FirechainZKEVMV2Factory.deploy(
            firechainZkEVMGlobalExitRoot.target,
            polTokenContract.target,
            firechainZkEVMBridgeContract.target,
            rollupManagerContract.target
        ...
        const newZkEVMContract = FirechainZKEVMV2Factory.attach(newZKEVMAddress) as FirechainZkEVMEtrog;
        ...
        const lastGlobalExitRootS = await firechainZkEVMGlobalExitRoot.getLastGlobalExitRoot();
        ...
        const rootSC = await firechainZkEVMGlobalExitRoot.getRoot();
        ...
        await expect(
            ...
            .to.emit(firechainZkEVMGlobalExitRoot, "UpdateL1InfoTree")
        ...
        expect(
            await firechainZkEVMBridgeContract.verifyMerkleProof(newLocalExitRoot, proofRollups, indexLeaf, rootRollups)
        ).to.be.equal(true);
    ...
    it("should check full flow upgrading rollup etrog", async () => {
        ...
        // Create zkEVM implementation
        const FirechainZKEVMV2Factory = await ethers.getContractFactory("FirechainZkEVMEtrogPrevious");
        const FirechainZKEVMV2Contract = await FirechainZKEVMV2Factory.deploy(
            firechainZkEVMGlobalExitRoot.target,
            polTokenContract.target,
            firechainZkEVMBridgeContract.target,
        ...
        // Create zkEVM implementation
        const FirechainZKEVMEtrogFactory = await ethers.getContractFactory("FirechainZkEVMEtrog");
        const FirechainZKEVMEtrogContract = await FirechainZKEVMEtrogFactory.deploy(
            firechainZkEVMGlobalExitRoot.target,
            polTokenContract.target,
            firechainZkEVMBridgeContract.target,
            rollupManagerContract.target
        );
        await FirechainZKEVMEtrogContract.waitForDeployment();
        ...
        await expect(
            rollupManagerContract
                .connect(timelock)
                .addNewRollupType(
                    FirechainZKEVMEtrogContract.target,
                    verifierContract.target,
                    forkID,
                    rollupCompatibilityID,
                    genesisRandom,
        ...
        // Validate upgrade OZ
        await upgrades.validateUpgrade(FirechainZKEVMV2Factory, FirechainZKEVMEtrogFactory, {
            constructorArgs: [
                firechainZkEVMGlobalExitRoot.target,
                polTokenContract.target,
                firechainZkEVMBridgeContract.target,
        ...
        await expect(
            rollupManagerContract
                .connect(timelock)
                .updateRollup(firechainZkEVMGlobalExitRoot.target, etrogRollupType, "0x")
    ...
    it("should check full flow upgrading validium storage migration", async () => {
        ...
        // Create zkEVM implementation
        const FirechainValidiumPreviousVersion = await ethers.getContractFactory("FirechainValidiumEtrogPrevious");
        const FirechainZKEVMV2Contract = await FirechainValidiumPreviousVersion.deploy(
            firechainZkEVMGlobalExitRoot.target,
            polTokenContract.target,
            firechainZkEVMBridgeContract.target,
            rollupManagerContract.target
        ...
        const newZkEVMContract = FirechainValidiumPreviousVersion.attach(newZKEVMAddress) as FirechainValidiumEtrogPrevious;
        const newSequencedBatch = 1;
        ...
        const FirechainDataCommitee = (await upgrades.deployProxy(FirechainDataCommiteeFactory, [], {
            unsafeAllow: ["constructor"],
        })) as any as FirechainDataCommittee;

        await newZkEVMContract.connect(admin).setDataAvailabilityProtocol(FirechainDataCommitee.target);

        expect(await newZkEVMContract.dataAvailabilityProtocol()).to.be.equal(FirechainDataCommitee.target);
        await FirechainDataCommitee.setupCommittee(0, [], "0x");
        ...
        // Create zkEVM implementation
        const FirechainValidiumStorageMigration = await ethers.getContractFactory("FirechainValidiumStorageMigration");
        const FirechainValidiumMigrationContract = await FirechainValidiumStorageMigration.deploy(
            firechainZkEVMGlobalExitRoot.target,
            polTokenContract.target,
            firechainZkEVMBridgeContract.target,
            rollupManagerContract.target
        );
        await FirechainValidiumMigrationContract.waitForDeployment();
        ...
        await expect(
            rollupManagerContract
                .connect(timelock)
                .addNewRollupType(
                    FirechainValidiumMigrationContract.target,
                    verifierContract.target,
                    forkID,
                    rollupCompatibilityID,
                    genesisRandom,
        ...
        await upgrades.validateUpgrade(FirechainValidiumPreviousVersion, FirechainValidiumStorageMigration, {
            constructorArgs: [
                firechainZkEVMGlobalExitRoot.target,
                polTokenContract.target,
                firechainZkEVMBridgeContract.target,
        ...
        await expect(
            rollupManagerContract
                ...
                .updateRollup(
                    newZKEVMAddress,
                    etrogRollupType,
                    FirechainValidiumStorageMigration.interface.encodeFunctionData("initializeMigration", [])
                )
        )
            .to.emit(rollupManagerContract, "UpdateRollup")
            .withArgs(newRollupTypeID, etrogRollupType, newVerifiedBatch);
        ...
        // const FirechainCurrentValidium = await ethers.getContractFactory("FirechainValidiumEtrog");
        // const FirechainCurrentValidiumContract = await FirechainCurrentValidium.deploy(
        //     firechainZkEVMGlobalExitRoot.target,
        //     polTokenContract.target,
        //     firechainZkEVMBridgeContract.target,
        //     rollupManagerContract.target
        // );
        // await FirechainCurrentValidiumContract.waitForDeployment();
        // await upgrades.validateUpgrade(FirechainValidiumStorageMigration, FirechainCurrentValidium, {
        //     constructorArgs: [
        //         firechainZkEVMGlobalExitRoot.target,
        //         polTokenContract.target,
        //         firechainZkEVMBridgeContract.target,
    ...
    it("should add existing rollup and test full flow", async () => {
        ...
        // Create zkEVM implementation
        const FirechainZKEVMV2Factory = await ethers.getContractFactory("FirechainZkEVMExistentEtrog");
        const FirechainZKEVMV2Contract = await FirechainZKEVMV2Factory.deploy(
            firechainZkEVMGlobalExitRoot.target,
            polTokenContract.target,
            firechainZkEVMBridgeContract.target,
        ...
        // Initialize:
        await expect(
            FirechainZKEVMV2Contract.initializeUpgrade(
                admin.address,
                trustedSequencer.address,
                urlSequencer,
                networkName,
        ...
        await expect(
            FirechainZKEVMV2Contract.connect(RollupManagerMock).initializeUpgrade(
                admin.address,
                trustedSequencer.address,
                urlSequencer,
                networkName,
        ...
        await expect(
            ...
            .withArgs(RollupID, forkID, FirechainZKEVMV2Contract.target, chainID, rollupCompatibilityID, 0);
        ...
        // Initialize upgrade
        await FirechainZKEVMV2Contract.connect(RollupManagerMock).initializeUpgrade(
            admin.address,
            trustedSequencer.address,
            urlSequencer,
            networkName,
        ...
        const txSetupEtrog = await FirechainZKEVMV2Contract.SET_UP_ETROG_TX();
        ...
        expect(await FirechainZKEVMV2Contract.lastAccInputHash()).to.be.equal(expectedAccInputHashInitial);
