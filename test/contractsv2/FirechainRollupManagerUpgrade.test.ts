...
describe("Firechain Rollup manager upgraded", () => {
    let deployer: any;
    let timelock: any;
    ...
    let firechainZkEVMContract: FirechainZkEVM;
    ...
    beforeEach("Deploy contract", async () => {
        ...
        expect(precalculatezkEVM).to.be.equal(firechainZkEVMContract.target);

        const FirechainRollupManagerFactory = await ethers.getContractFactory("FirechainRollupManagerMock");
        rollupManagerContract = FirechainRollupManagerFactory.attach(firechainZkEVMContract.target) as any;
        ...
        const newFirechainZkEVMContract = (await upgrades.deployProxy(FirechainZkEVMV2ExistentFactory, [], {
            ...
            unsafeAllow: ["constructor", "state-variable-immutable"],
        })) as any as FirechainZkEVMExistentEtrog;
    ...
    it("should check full flow no trusted aggreagtor", async () => {
        ...
        expect(
            await firechainZkEVMBridgeContract.verifyMerkleProof(newLocalExitRoot, proofRollups, indexRollup, rootRollups)
        ).to.be.equal(true);
